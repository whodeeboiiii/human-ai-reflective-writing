...
export async function GET(req: Request): Promise<NextResponse> {
  ...
  if (!clientId || !clientSecret) {
    ...
    return NextResponse.json({ bookContext: null });
  }

  try {
    const url = `https://openapi.naver.com/v1/search/book.json?query=${encodeURIComponent(query)}&display=1`;
    ...
    console.log(
      `[book-search] query="${query}" → 책="${bookContext.title}" · 저자="${bookContext.author}" · 출판사="${bookContext.publisher}"`
    );

    return NextResponse.json({ bookContext });
  } catch (err) {
